...
WHY LoRA (Low-Rank Adaptation)?
   ...
   each attention layer. Specifically, for a weight matrix W (shape dxd):
     Instead of updating W, we learn two small matrices A (dxr) and B (rxd)
     where rank r=8. The update is: W + AxB
...
TRAINING SETUP:
   ...
   Precision:  bfloat16 on GPU (avoids the FP16 gradient-scaler error seen with PEFT)
...
EPOCHS        = 3
BATCH_SIZE    = 4
GRAD_ACCUM    = 4      # effective batch = BATCH_SIZE x GRAD_ACCUM = 16
LEARNING_RATE = 2e-5
LORA_RANK     = 8      # LoRA r -- higher = more capacity, more VRAM
LORA_ALPHA    = 32     # LoRA scaling factor (convention: alpha = 4 x rank)
LORA_DROPOUT  = 0.05
...
def finetune():
    ...
    training_args = TrainingArguments(
        ...
        warmup_ratio                = 0.05,
        bf16                         = (device == "cuda"),  # avoids FP16 grad-scaler ValueError seen with PEFT
        fp16                         = False,
        logging_steps               = 50,
        save_strategy               = "epoch",
